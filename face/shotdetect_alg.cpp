...
int mainStaticMatch(char* path1, char* path2, int size)
{
  ...
  if (surf_vec_map.find(str_path2) != surf_vec_map.end())
  {
	printf("[%s] - %d - %x\n", path2, surf_vec_map[str_path2].size(), &surf_vec_map[str_path2]);
	getMatches(ipts1, surf_vec_map[str_path2], matches);
	printf("Matches: %d\n", matches.size());
...
void *surf_detect_callback(void *argv)
{
	...
	char imgpath[1024];
	memset(imgpath, 0, 1024);
	sprintf(imgpath, "%sdata/%s/scene", strFilePath.c_str(), ctx->inputpath2);
	printf("scene - %s\n", imgpath);
	int surf = 0;	// when surf_detect-return 5; ,surf = 5;
	int count = 0;
...
surf_open_dir:
	...
	else
	{
		...
		while ((entry = readdir(directory_pointer)) != NULL)
		{
			...
			memset(tmppath, 0, 1024);
			sprintf(tmppath, "%sdata/%s/scene/%s", strFilePath.c_str(), ctx->inputpath2, entry->d_name);
...
extern "C" map<int, int> surf_detect_mutiple(int multiple_cnt, char* scene, char* filename)
{
	...
	char imgpath[1024];
	memset(imgpath, 0, 1024);
	sprintf(imgpath, "%sdata/%s/images/%s", strFilePath.c_str(), scene, filename);
	printf("images - %s\n", imgpath);
	
	char cmd[1024];
	memset(cmd, 0, 1024);
	sprintf(cmd, "rm -rf %sdata/%s/images/%s/", strFilePath.c_str(), scene, filename, filename);
	printf("cmd - %s\n", cmd);
	...
	else 
	{
		...
		while ((entry = readdir(directory_pointer)) != NULL)
		{
			...
			memset(tmppath, 0, 1024);
			sprintf(tmppath, "%sdata/%s/images/%s/%s", strFilePath.c_str(), scene, filename, entry->d_name);
			printf("img tmppath - %s\n", tmppath); 
...
extern "C" int facedect_seeta(char* imgpath)
{
	int face_cnt = 0;
	char cmd[1024];
	memset(cmd, 0, 1024);
	...
	// 人脸识别
	sprintf(cmd, "facedet_image %s %smodel/seeta_fd_frontal_v1.0.bin | grep face_cnt | awk '{ print $2 }'", imgpath, getFilePath());
...
extern "C" double facematch_seeta(char* imgpath, char* filename, char* labelpath)
{
	double sim = 0.0;
	char cmd[1024];
	memset(cmd, 0, 1024);
	...
	// 人脸识别
	sprintf(cmd, "facematch_seeta %s %s %s | grep face_match | awk '{ print $2 }' | tail -n 1", imgpath, filename, labelpath);
...
extern "C" int caffe_detect(char* imgpath)
{
	int caffe_class = -1;
	float caffe_conf = -1.0;
	char cmd[1024];
	memset(cmd, 0, 1024);
	
	string strFilePath = getFilePath();
	// 人脸识别
	sprintf(cmd, "caffe_classification %smodel/googlenet/deploy_googlenet_samples1_224.prototxt %smodel/googlenet/googlenet_samples1_train_224_iter_15000.caffemodel %smodel/googlenet/mean/mean_224.binaryproto %smodel/words_1000.txt %s | grep -v 'Prediction for ' | head -n 1 | awk '{ print $1 $3 }'", 
		strFilePath.c_str(), strFilePath.c_str(), strFilePath.c_str(), strFilePath.c_str(), imgpath);
	...
	{
		str = tmp;
		caffe_conf = atof(str.substr(0, str.find_last_of("\"")).c_str());
		caffe_class = atoi(str.substr(str.find_last_of("\"") + 1, str.length() - str.find_last_of("\"") - 1).c_str());
		memset(tmp, 0, 128);
	}
...
map<int, int> caffe_map;
map<int, string> caffe_map_error;
typedef struct caffe_detect_context
{
	char *inputpath1;
} caffe_detect_context;

void *caffe_detect_callback(void *argv)
{
	caffe_detect_context* ctx = (caffe_detect_context*)argv;
	string str = ctx->inputpath1;
	string strname = str.substr(0, str.find_last_of("_"));
	string stridr = str.substr(str.find_last_of("_") + 1, str.find_last_of(".") - 1 - str.find_last_of("_"));
	//printf("[%s][%s]\n", strname.c_str(), stridr.c_str());
	
	int conf = 0;	// when conf_detect-return 1; ,conf = 1;
	int val = 0;
	int cnt = 1;
	while (1)
	{
		val = caffe_detect(ctx->inputpath1);
		if (val >= 0 || cnt >= 3)
			break;
		cnt++;
		if (val < 0)
	...
	{
		long idr = atol(stridr.c_str());
		caffe_map_error[idr] = string(ctx->inputpath1);	
	}
	conf = conf > val ? conf : val;
	fflush(stdout);
...
extern "C" map<int, int> caffe_detect_mutiple(int multiple_cnt, char* scene, char* filename)
{
	string strFilePath = getFilePath();
	...
	else 
	{
		...
		while ((entry = readdir(directory_pointer)) != NULL)
		{
			...
			int ret = pthread_create(&thr, &attr, caffe_detect_callback, ctx);
			pthread_attr_destroy(&attr);
		...
		/// recalculate error
		typedef map<int, string>::iterator IT;
		for(IT it = caffe_map_error.begin(); it != caffe_map_error.end(); it++) 
		{
			printf("%s\n", it->second.c_str());
